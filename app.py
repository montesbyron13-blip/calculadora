...
# Inicializar estado de sesión para los valores
...
def generar_imagen_resultados(datos_local, datos_total):
    """Genera una imagen PNG con los resultados"""
...
def generar_html_ticket(datos_local, datos_total):
    """Genera un HTML con formato de ticket térmico (80mm)"""
    now = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    html = f"""
    <html>
    <head>
...
# Preparar diccionarios para las funciones
datos_local = {
    'com_fisicas': com_fisicas,
    'pos': pos,
    'caja_ini': caja_ini,
    'salidas': salidas,
    'efectivo_cont': efectivo_cont,
    'depositos': depositos,
    'ventas_efectivo': ventas_efectivo,
    'efectivo_ideal': efectivo_ideal,
...
st.download_button(
    ...
    data=buf,
    file_name=f"cierre_caja_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png",
    mime="image/png"
)
plt.close(fig)

# Imprimir ticket (HTML + botón de impresión)
st.subheader("🖨️ Imprimir ticket térmico")
html_ticket = generar_html_ticket(datos_local, datos_total)

# Opción 1: Descargar HTML
b64 = base64.b64encode(html_ticket.encode()).decode()
href_html = f'<a href="data:text/html;base64,{b64}" download="ticket_cierre.html">📄 Descargar HTML (para imprimir después)</a>'
st.markdown(href_html, unsafe_allow_html=True)

# Opción 2: Imprimir directamente con window.print()
...
st.info("💡 Al hacer clic en 'Imprimir ticket ahora', el navegador abrirá la vista previa. Selecciona tu impresora térmica Bluetooth/WiFi desde allí. Si no ves la impresora, asegúrate de que esté emparejada con tu dispositivo.")
